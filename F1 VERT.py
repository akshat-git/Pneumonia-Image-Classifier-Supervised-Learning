...
# -------------------
def compute_precision_recall_f1(all_labels, all_probs):
    precision, recall, thresholds = precision_recall_curve(all_labels, all_probs, pos_label=1)
    f1_scores = 2 * (precision * recall) / (precision + recall + 1e-10)  # Avoid division by zero
    best_idx = np.argmax(f1_scores)
    return precision, recall, thresholds, f1_scores, best_idx


# -------------------
# Plot Precision-Recall-F1 Overlay
# -------------------
def plot_precision_recall_f1(all_data_dict):
    plt.figure(figsize=(8, 6))
    for model_name, (precision, recall, f1_scores, thresholds, best_idx) in all_data_dict.items():
        plt.plot(recall, precision, label=f'{model_name} PR Curve')
        # Mark the best F1 point
        plt.plot(recall[best_idx], precision[best_idx], 'ro', label=f'{model_name} Best F1')
        plt.text(
            recall[best_idx], precision[best_idx],
            f'F1={f1_scores[best_idx]:.3f}\nT={thresholds[best_idx]:.3f}',
            fontsize=9, color='red'
        )

    plt.xlabel('Recall')
    plt.ylabel('Precision')
    plt.title('Precision-Recall Curves with F1 Scores')
...
# -------------------
def adjust_class_distribution(dataset, target_ratio):
    """Adjust dataset to target class ratio by oversampling or undersampling."""
    class_counts = np.bincount([label for _, label in dataset])
    total_samples = sum(class_counts)
    target_samples = [int(total_samples * ratio) for ratio in target_ratio]

    new_data = []
    for cls, target_count in enumerate(target_samples):
        class_data = [sample for sample in dataset if sample[1] == cls]
        if len(class_data) < target_count:  # Oversample
            class_data = class_data * (target_count // len(class_data)) + class_data[:target_count % len(class_data)]
        else:  # Undersample
            class_data = class_data[:target_count]
        new_data.extend(class_data)

    return new_data

# -------------------
# Simulate Imbalances and Compute Metrics
# -------------------
imbalances = {
    "Original": None,  # Use original data distribution
    "80-20": [0.8, 0.2],
    "50-50": [0.5, 0.5]
}
...
for scenario, ratio in imbalances.items():
    print(f"\nSimulating imbalance: {scenario}")

    if ratio:
        balanced_data = adjust_class_distribution(train_data, ratio)
        balanced_loader = DataLoader(balanced_data, batch_size=CFG.batch_size, shuffle=True)
    else:
        balanced_loader = train_loader

    # Train model with balanced data
    model = create_model("ResNet18", num_classes=2, freeze=True)
    optimizer = optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=CFG.lr)

    for epoch in range(CFG.epochs):
        train_epoch(model, balanced_loader, criterion, optimizer)

    # Test and compute precision-recall metrics
    _, _, all_labels, all_probs = test_model(model, test_loader, criterion)
    precision, recall, thresholds, f1_scores, best_idx = compute_precision_recall_f1(all_labels, all_probs)

    # Compute Youden's J for each scenario
    fpr, tpr, roc_thresholds, _ = compute_roc_info(all_labels, all_probs)
    thresholds, j_vals, best_thresh, best_j = compute_youdens_j(fpr, tpr, roc_thresholds)

    pr_f1_data[scenario] = (precision, recall, f1_scores, thresholds, best_idx)
    j_data[scenario] = (thresholds, j_vals, best_thresh, best_j)

# -------------------
# Plot F1 and Precision-Recall
# -------------------
plot_precision_recall_f1(pr_f1_data)